...
<c:if test="true">true<br></c:if>
<c:if test="TRUE">TRUE<br></c:if>
<c:if test="True">True<br></c:if>
<c:if test="trUe">trUe<br></c:if>
<%-- 그외의 모든 문자열은 거짓으로 간주한자. --%>
<c:if test="false">false<br></c:if>
<c:if test="aaaa">aaaa<br></c:if>

<%-- 값을 비교할 경우 그냥 일반 문자열로 표현하면 위의 규칙에 따라 거짓으로 간주된다. --%>
<c:if test="name == 유관순">유관순1111<br></c:if>
