...
/* URL 인코딩?
 * => 7비트만 처리할 수 있는 장비를 사용하는 경우,
 *    8비트 데이터를 보낼 수 있게 만드는 인코딩 기술이다.
 * => 8비트 값을 7비트화 시키는 것을 "URL 인코딩" 이라 부른다.
 * => 방법
 *    예) 보내려는 데이터가 영어 알파벳 'A'라고 가정하자!
 *        'A'의 코드 값 : 0x41 (0100 0001)  <-- 7비트 장비를 통과하더라도 데이터를 잃지 않는다.
 *        '가'의 코드 값: 0xAC00(10101100 0000000) <-- 7비트 장비 통과후 데이터가 깨진다.
 *    해결)
 *        '가'의 코드 값(0xAC00)을 ASCII 문자화 시킨다.
 *         유니코드 값을 그대로 ASCII 문자화 시킨다면,
 *          => %AC%00: '%'->25 'A'->41 'C'->43 '%'->25 '0'->30 '0'->30
 *         2 바이트를 보내면 될 것을 ASCII 문자화 시키면 6바이트를 보내야 한다.
 *         보내는 데이터의 크기는 커지지만 7비트 장비를 무사히 통과할 수 있다.
 * => GET 또는 POST 요청할 때 보내는 데이터는 웹브라우저가 자동으로 URL 인코딩한다.
