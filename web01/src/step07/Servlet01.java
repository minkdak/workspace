/* 웹 애플리케이션에서 자원을 준비하는 방법: 서블릿을 이용하기 I
 * => 다른 서블릿을 실행하기 전에 자원을 준비시키는 서블릿을 먼저 실행하여
 *    자원을 준비시키는 방법
 * => 이 서블릿의 문제점?
 *    - init() 메서드에서 자원을 준비한다.
 *    - init() 메서드는 서블릿 객체가 생성할 때 호출된다.
 *    - 서블릿 객체는 클라이언트로부터 최초의 요청이 들어왔을 때 생성된다.
 *    - 클라이언트에서 요청하지 않으면 생성되지 않는다.ㅇㅋ?
 *    - 생성되지 않으면 init() 호출되지 않는다.
 *    - 따라서, 서버 운영자가 서버를 껐다가 켤 때 마다 이런 작업을 수행하는 서블릿을
...
public class Servlet01 extends HttpServlet {
  ...
  public void init() throws ServletException {
    ...
    // 이 메서드를 오버라이딩 하라고 GenericServlet 클래스에서 재공해준다.
    // 개발자는 서블릿이 생성될 때 뭔가 준비 시키고 싶다면,
