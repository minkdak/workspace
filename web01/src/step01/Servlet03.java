 ...
 *      오버라이딩 할 메서드가 결정된다.
 *    - 예를 들면 
 *      클라이언트의 GET요청만 처리하고 싶다면 doGet()을 오버라이딩하고,
 *      클라이언트의 POST처리하고 싶다면 doPost()를 오버라이딩 한다.
 *      만약 둘 다 처리하고 싶다면 doGet(), doPost() 모두 오버라이딩 하면 된다.
 *      
 * doGet(), doPost() 호출 테스트
 * => /WebContent/servlets/step01/Servlet03.html로 GET 요청과 POST 요청을 실행한다.
 */
package step01;
...
public class Servlet03 extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    // Servlet03에 대해 GET 요청이 들어온다면 서블릿 컨테이너는 doGet()을 호출할 것이다.
    // 서블릿 컨테이너
