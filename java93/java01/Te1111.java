/* 자바 프로그래밍 - 소개*/
class Test01 { // 분류명 파일명과 같게 맞춘다
  public static void main(String[] args){ //public 이라는 공개 ??에 정적인 파일로/ 리턴값은 없고/main/
...
- 자바 바이트코드(bytecode)를 실행할 때 사용하는 프로그램들이 들어 있다.
- 핵심 구성 요소
  => Java Virtural Machine(JVM): bytecode 실행 프로그램
...
- 기존의 JRE에서 데스크탑 애플리케이션 관련 기능을 제외시키고,
  대신 서버 애플리케이션 관리에 필요한 모니터링 기능을 추가하였다.
- 일반인들이 사용하는 소프트웨어는 아니다.

자바 컴파일러와 JVM
1) 자바 컴파일러
- 자바 소스 코드를 JVM이 이해할 수 있는 bytecode로 변환시키는 프로그램.
...
- bytecode를 읽어서 OS의 코드(기능)를 +을 한다.
...
- natice 코드를 실행하는 것에 비해 실행 속도가 느리다.
...
- 컴파일이라는 과정을 거치지 않기 때문에 프로그램을 배포하기가 쉽다.
  즉 프로그램을 변경할 때 마다 컴파일 할 필요가 없어 바로 배포할 수 있다.
- 소스를 바로 실행하면,
  매번 실행할 때마다 소스코드를 분석해야 하고,
  문법의 유효성을 검사해야 한다.
  즉 실행속도가 느리다.
...
- bytecode를 실행하면,
  컴파일 과정에서 소스코드를 분석하고, 문법의 유효성도 검사하기 때문에
  실행할 때는 이 과정을 수행할 필요가 없어서 실행 속도가 빠르다.
  실행할 때도 VM 입장에서 소스코드 보다 좀 더 이해하기 쉬운 기계어에 가까운
  bytecode를 실행하는 것이 속도가 빠르다.

3) native 코드 실행하기
- OS의 형식에 맞춰서 작성된 완전한 기계어로 작성된 코드이기 때문에
...
컴파일과 실행
1) 컴파일
- 자바 소스코드를 바이트코드(bytecode)로 만드는 것
