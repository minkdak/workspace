/* 멀티 태스킹 : 스레드 만들고 실행하기
 * => 방법
 *    1) Thread를 상속 받아서 만들기 : 총 + 총알(폰 + 배터리)
 *       만들기:   class MyThread extends Thread {}
 *       실행하기: new MyThread().start()
 *    2) Runnable 규칙에 따라 만들기 : 총알 따로, 총따로
...
public class Test02 {
  static class MyThread extends Thread {
    ...
    // 그리고 run() 메서드 안에 "main" 스레드와 병행하여(동시에) 실행할 코드는 반드시
    // 반드시 run() 메서드를 오버라이딩 하여 그 메서드에 둔다.
    public void run() {
  ...
  public static void main(String[] args) {
    ...
    //2) Runnable 규칙ㄱ에 따라 만든 클래스를 스레드로 실행하기
