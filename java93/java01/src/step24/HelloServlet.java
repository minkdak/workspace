 ...
 *    => mvnrepository.com 에서 "servlet-apit"를 검색한다.
 *    => 3.1.x 버전의 gradle 라이브러리 정보를 복사한다.
 *    => bulid.gradle 파일의 dependencies {} 블록에 추가한다.
...
public class HelloServlet implements Servlet{
  ...
  public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
    System.out.println("service()"); // 콘솔창 출력.
    
    res.setContentType("text/plain;charset=UTF-8");
    PrintWriter out = res.getWriter();  // 이 객체에서 라이트를 꺼내서 써야한다.
    out.println("Hello, world!"); // 클라이언트 브라우저에서 출력
  }
