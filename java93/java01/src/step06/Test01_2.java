/* class 문법의 용도: 사용자 정의 데이터 타입 만들기II
 * => 객체를 다루는 코드를 별도의 메서드로 분리하면,
 *    필요할 때 마다 재사용할 수 있어 편리하다. 
 */
package step06;
...
public class Test01_2 {
  ...
  static class Student {
    String name;
    int[] scores = new int[3];// scores 라는 int 배열을 정의하고 배열 3개를 만든다.
    int total;
    float aver;
  }
  
  // 인스턴스와 그 인스턴스의 각 변수에 들어갈 값을 파라미터로 넘기면,
  // 해당 인스턴스의 각 변수에 파라미터로 받은 값을 저장한다.
  public static void init(Student s, String name, int kor, int eng, int math) {
    s.name = name; //s.name = "홍길동";
    s.scores[0] = kor;
    s.scores[1] = eng;
    s.scores[2] = math;
  }
  ...
  // 해당 인스턴스의 합계와 평균을 구해서 그 인스턴스 변수에 저장한다. 
  public static void compute(Student s) {//계산할 객체 주소 주세요
