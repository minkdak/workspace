...
module.exports = {
  ...
  selectOne(no, successFn, errorFn) {
    this.connection.query(
      'select m.mno, m.name, mr.posi, m.tel, mr.fax, m.email, mr.path \
      from mgr mr inner join memb m on mr.mrno=m.mno \
      where mr.mrno=?',
      [no],
      function(error, results) {
  ...
  insert(manager, successFn, errorFn) {
    this.connection.query(
      ...
      [ manager.no, manager.position, manager.fax, manager.path],
  ...
  update(manager, successFn, errorFn) {
    this.connection.query(
      ...
      [manager.position, manager.fax, manager.path, manager.no],
      function(error, result) {
        if (error) {
          errorFn(error)
        } else {
