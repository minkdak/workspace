...
handlerMapping['/get.do'] = (request, response) => { //url을 프로퍼티 이름으로 쓴다.
...
const server = http.createServer(function(request, response) {
  ...
  if(request.method == 'GET') { //GET요청일 때만 파라미터 값을 request에 보관한다.
...
// 앞으로 서비스(url)를 추가하더라도 아래 조건문을 손댈 필요가 없다.
  var fn = handlerMapping[urlInfo.pathname] // 값이 같으면 객체 리턴 아니면 undefined 리턴
  if(fn != undefined){
    fn(request, response)
  } else {
