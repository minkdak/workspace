...
=> DBMS 도입 후
  - 개발자는 File I/O 프로그래밍을 하지 않고
    대신 DBMS에게 명령(SQL)을 내려서 데이터를 처리하는 방법을 사용한다.
  - DBMS가 개발자를 대신하여 File I/O를 처리한다.

SQL(Structured Query Language)
=> 데이터를 처리할 때 DBMS에게 내리는 명령의 문법
=> select.. from.. where 와 같이 명령어의 문법이 이해하기 쉽도록 구조화 되어있다.
=> SQL은 DBMS와 별도로 정의된 표준 질의 언어이다.
  그러나 DBMS마다 고유의 문법이 존재한다.


NodeJS와 DBMS
=> 특정 DBMS와 연결하여 SQL을 전달하고 결과륿 받을 수 있는 서드파티 모듈이 존재한다.
  NodeJS에게 기본으로 제공하는 모듈은 아니다.
=> 사용하려면, DBMS 제품에 맞는 모듈을 설치해야한다.
...
*/
const mysql = require('mysql') // mysql 모듈 로딩 -> 한번사용하면 바뀌지 않으니까 var대신
