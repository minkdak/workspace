  ...
  connection.query(
    ...
    function(error, results) {
      var student = results[0]
        if (error) {
          errorFn(error) //error함수를 담고있는 주소호출
        } else {
          successFn(result[0])
        }
  }) //connection.query()
...
function insertStudent(student, successFn, errorFn) {// student 배열로받아도되는데 객체로 받자.
  connection.query(
    'insert into stud(sno,work,schl,_nm) values(?,?,?)',
...
function insertMember(member, successFn, errorFn ) {// student 배열로받아도되는데 객체로 받자.
  connection.query(
