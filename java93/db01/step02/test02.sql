...
char(x):
 ...
 => 한 개의 문자를 입력하면 나머지 개수만큼 빈문자가 채워진다.
...
varchar(x)
 ...
 => char(x)대신 용량을 최적화하는 대신 데이터 찾는데 시간이 걸린다.
 => 최대 255자 까지
text
...
longtext
 => 최대 2**32 자까지(약 42억자)까지 입력할 수 있다.
date
 => 년,월,일 날짜 입력
datetime
...
/* varchar(10)=> 12345678901 입력시 크기가 초과되어 오류 발생. */
insert into t2(no, name, gender, height, intro, birthday, regdate)
...
/* height numeric(4, 1) => 최대 4자리 숫자. 소수점 이하 한 자리만 가능
=> 소수점 이하 2자리면는 반올림 처리됨.
*/
insert into t2(no, name, gender, height, intro, birthday, regdate)
...
*/
insert into t2(no, name, gender, height, intro, birthday, regdate)
values(6, '홍길동', 'W', 2060.6, '길순이...', '1993-5-5', '2017-4-17 15:23:11');

/* date => 날짜 정보만 저장됨
=> 시간 정보를 입력해도 날짜 정보만 저장된다.
*/
insert into t2(no, name, gender, height, intro, birthday, regdate)
values(7, '동길순', 'W', 150, '길순이...', '1993-5-5 7:3:5', '2017-4-17 15:23:11');

/* datetime =>

*/
insert into t2(no, name, gender, height, intro, birthday, regdate)
values(8, '동길순', 'W', 150, '길순이...', '1993-5-5', '2017-4-17 ');
