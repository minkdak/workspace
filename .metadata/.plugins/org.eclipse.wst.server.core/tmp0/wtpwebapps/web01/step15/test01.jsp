...
=> 결론
   ...
   - JSP가 직접 실행되는 것이 아니라 JSP로 만든 서블릿 클래스가 실행되는 것이다.
   - JSP가 직접 실행되지 않는다.
=> JSP 기술이 등장한 이유?
   - 서블릿에서 콘텐츠를 출력하려면 I/O 스트림 클래스를 사용하여 출력함수를 호출해야 한다.
...
public final class test01_jsp extends org.apache.jasper.runtime.HttpJspBase
    implemnte org.apache.jasper.runtime.JspSourceDependent,
                ?
                
=> _jspService() 메서드?
   웹브라우저 요청 => 서블릿 컨테이너가 
     => 서블릿 컨테이너가 service() 메서드 호출
     => service() 에서 _jspServcice() 메서드 호출

=> JSP 엔진이 JSP 파일을 가지고 서블릿 클래스를 만들때 반드시 지켜야 하는 규칙?
   => 추가로 javax.servlet.jsp.HttpJspPage 인터페이스를 구현해야 한다.
   => HttpJspPage?
