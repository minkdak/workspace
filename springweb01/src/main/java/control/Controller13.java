...
public class Controller13 {
  ...
  // 단 원시 타입(byte,short,int,long,float,double,boolean,char)으로만 변환해 준다.
  // 다른 타입으로 변환하고 싶다면, 별도의 변환기를 설치해야 한다.
  
  // 1) String ---> java.sql.Date 으로 자동 변환 가능!
  @RequestMapping("ok1")
  public void ok1(java.sql.Date date, HttpServletResponse response) throws Exception {
  ...
  @RequestMapping("ok2")
  public void ok2(
      String name, // @Requestparam이 붙지 않으면 자동으로 선택항목이 된다.
      java.util.Date date, 
      boolean working,
  ...
  // @InitBinder가 붙은 메서드는 
  // 클라이언트에서 보낸 각각의 요청 파라미터에 대해 요청 핸들러의 아규먼트 값으로 바꿀 때 마다 매번 호출된다. 
  // => 기능? 요청 파라미터를 아규먼트 값으로 바꿔주는 변환기를 사용한다.
  @InitBinder
  protected void initBinder(WebDataBinder binder) { // 한번 등록하고 끝이 아니라 매번 호출된다.
      ...
      binder.registerCustomEditor(
          java.util.Date.class, // 첫번 째 파라미터: 문자열을 변환하여 만들 값의 타입
          // 이런타입으로 바꿔야 한다면 아래 에디터를 사용하라
