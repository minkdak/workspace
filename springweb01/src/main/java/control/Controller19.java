...
public class Controller19 {
  ...
  @RequestMapping("ok2")
  public String ok2(String name, HttpSession session, Model model) throws Exception {
  ...
  public String reset(HttpSession session, SessionStatus status) throws Exception {
    // setComplete()
    // => 현재 페이지 컨트롤러에서 @SessionAttributes에 설정된 객체만 세션에서 제거한다.
    // status.setComplete(); // setAttribute로 보관한 적이 없다면 이걸로 날릴 수 있다.
    
    // HttpSession.setAttribute()로 설정된 객체를 지우려면,
    // 1) 세션 전체를 무효화시킨다.
     session.invalidate(); // setAttribute로 보관했다면 이걸 사용하여 날려야 한다.
