 ...
 *    - 이런 방식은 기능을 추가할 때 마다 메서드를 추가하는 것 보다
 *      메서드를 추가하는 것 보다 클래스를 추가하는 것이 유지 보수를 더 쉽게 해주기 때문이다. 
 *    - 작업
 *      1. 호출자(HttpProcessor)와 호출당하는자(요청처리객체) 사이의 호출 규칙을 정의한다.
 *         => Command 인터페이스 정의
 *      2. 메서드를 추출하여 Command 규칙에 따라 클래스를 만든다.
 *         => HelloCommand, OkServlet, ErrorServlet 작성
...
public class HttpServer {
  ...
  public HttpServer(int port) { // 서버 생성자
    this.port = port;
