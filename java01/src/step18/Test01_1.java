 ...
 *       - 대기열이 꽉 차있을 경우, 클라이언트의 연결 요청을 거절한다.
 * => 종류
 *    1) 서버 소켓
...
public class Test01_1 {
  public static void main(String[] args) throws Exception{
    ...
    // 2) 대기열의 첫 번째 클라이언트를 접속을 승인한다.
    //  => 만약 대기 중인 클라이언트가 없다면, 리턴하지 않고 접속할 때 까지 기다린다.
    //  => 여러 클라이언가 대기 중이라면 그 중 첫 번째 클라이언트와 연결한다.
    ...
    int b = in.read(); // 클라이언트에서 1바이트를 보낼 때 까지 계속 기다린다(blocking)
    System.out.printf("%x\n", b);
    
    out.write(0x01); // 서버가 보낸????????리턴하지 않는다?
