...
public class Test13 {
  ...
  public static void main(String[] args) {
    ...
    //1) 인스턴스 주소를 저장할 레퍼런스 배열을 만든다.
    Student[] arr1 = new Student[3]; // 레퍼런스를 3개 만든다. 
                                     // 인스턴스를 3개 만드는 것이 아니다.
    
    //2) Student 인스턴스를 생성한 후 그 주소를 레퍼런스 배열에 저장한다.
    arr1[0] = new Student(); // new를 안써도 Student라는 메서드를 담을 수 있는가?
    arr1[1] = new Student(); // Student 클래스에 주소값을 리턴해주는 리턴값이 있으면 가능하다.
    arr1[2] = new Student(); // 없다면 new라는 명령어를 작성해주어야 한다.
    // 인스턴스 배열을 만드는 문법은 자바에 없다.
