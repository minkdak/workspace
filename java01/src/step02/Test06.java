...
public class Test06 {
  ...
  public static void main(String[] args) {
   ...
   // byte arr1[]; //C언어에서 주로 사용하는 문법도 지원한다.
    /* 설명
     * => byte[] arr1
    ...
    //arr1[0] = 10; // 아직 arr1 변수의 값을 초기화시키지도 않고 사용하면 컴파일 오류!
    /* 주의!
  ...
  //arr1 = null; //(null은 일반 변수에 사용할 수 없다. 오직 레퍼런스 변수에만 사용할 수 있다.)
    ...
    //arr1[0] = 10; // null로 초기화시켰다 하더라도 일단 변수의 값은 초기화 된 것이기 때문에
                  // 컴파일 오류는 발생하지 않는다.
                  // 다만, 실행할 때 arr1 변수의 주소가 없기 때문에 
                  // 실행 오류가 "Runtime Exception"가 발생한다.
   ...
   arr1[3] = 40; // 존재하지 않는 메모리의 인덱스 가리키더라도 컴파일 오류는
     // 그러나 실행할 때 그 인덱스에 해당하는 메모리가 없기 때문에 실행 오류가
  }
