/* : 사용자 정의 데이터 타입: 생성자와 메서드
 * => 인스턴스를 생성한 후 자동으로 호출되는 특별한 메서드
 * => 그래서 주로 인스턴스를 사용하기 전에 그 메모리를 초기화 시키는 용도로 사용한다.
...
public class Test16 {
  ...
  public static void main(String[] args) {
    ...
    //    생성자 호출하지 않을 수 있는가? 없다! 무조건 호출해야 한다.
     ...
     * => 클래스와 관련된 코드는 클래스 안에 묶어 두는 것이 유지보수에 좋다.
     * => 나중에 클래스 별로 별개의 파일로 분리할 때 유리하다.
     *    유지보수 입장에서는 코드를 관리하기 쉽다.      
     */
    
    // printStudent() 메서드 또한 이 메서드와 관련된 클래스 안에 옮겨둔다.
