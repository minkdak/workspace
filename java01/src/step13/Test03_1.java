...
public class Test03_1 {
  ...
  public static void main(String[] args) {
    ...
    /*
     main() 은 static 멤버이다. 인스턴스를 생성하지 않고 바로 호출하는 메서드 이다.
     그래서 인스턴스 없이 호출하기 때문에, 그래서 인스턴스 멤버를 직접 사용할 수 없다.
    ...
    */
    ...
    // 사용하는 방법은?
    // => 잊지마세요. 인스턴스 멤버입니다. 인스턴스 멤버는 인스턴스를 만든 후에
    //    언제든지 사용할 수 있습니다.
    obj.a = 20; //OK
    obj.m1(); //OK
    obj.test(); //OK
    ...
    /* 설명
     * => 참조 변수를 선언할 때는 인스턴스 주소가 없어도 된다.
     * => 그러나 객체를 생성할 때는 인스턴스 주소를 앞에 두고 생성한다.참조변수 선언 = new 수퍼클래스이름(생성자호출 파라미터) {...}
     */
