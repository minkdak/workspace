 ...
 *    자동으로 Object를 상속 받는다.
 * => 왜 자바의 모든 클래스는 Object의 sub클래스로 만들까?
 *    자바 객체(인스턴스)를 다루기 쉽게 하기 위해서이다.
 * => 하는 일?
 *    => 객체가 기본적으로 갖춰야할 변수와 메서드를 제공하는 역할!
 *    => 주요 메서드
 *       toString() : 클래스 이름과 인스턴스 ID를 리턴한다.
 *                    => 프로그램 실행할 때 인스턴스의 내부 값을 간단히 살펴볼 수 있도록
 *                       쉽게 하기 위해 서브 클래스들은 이 메서드를 재정의 하기도 한다.
 *       equals()   : 인스턴스의 주소를 비교하여 true/false를 리턴한다.
 *                    => 인스턴스 내부의 값을 비교하도록
...
public class Test03_1 /* extends Object */{ // 컴파일할 때 자동으로 붙음
