 ...
 *    그 코드가 같은 인스턴스 변수의 값을 변경하는 것이아니라 단지 조회만 하는 경우
 *    이런 경우에는 데이터가 중복되는 문제가 발생하지 않는다.
 *    이렇게 여러 스레드가 동시에 실행하더라도 안전한 코드 블록을
 *    "스레드 안전(Thread Safe)하다" 표현한다.
 *    
 * => 여러 스레드가 크리티컬 섹션을 실행할 때 문제가 발생하지 않게 만드는 방법
 *    - 동시에 실행하지 않게 만들면 된다.
 *    - 한 번에 한 스레드 만이 그 크리티컬 섹션에 코드를 실행하게 하면 된다.
 *    - 어떻게? 
...
public class Test05_1 {
  ...
  static class Account {
    ...
    // 여러 ATM스레드가 이 메서드를 호출한다.
    // 이 메서드 안에서는 같은 인스턴스의 변수 값을 변경한다.
    // 문제는 여러 스레드들이 같은 인스턴스에 대해 withdraw() 메서드를 호출 했을 때
    // 서로 중복하여 값을 변경할 수 있다는 것이다.
    // 이렇게 여러 스레드가 동시에 접근해서 실행했을 때
    // 문제가 발생하는 코드 블럭을 "Critical Section"이라 부른다.
    public long withdraw(long money) {
      long temp = this.balance;
      
      // 약간의 시간 지연을 발생시킨다.
      // 왜? 일부러 다른 스레드에게 CPU 사용권을 뺏기기 위함.
      double x = 3.141592;
      int delayCount = (int)(Math.random() * 10); // 인트 붙여서 소수점 떼어냄
